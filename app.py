...
class User(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    # Self-signup: False until email OTP verified (6-digit code sent via SMTP).
...
def _verification_code_pepper() -> str:
    return (
        ...
        or _normalize_secret_value(_secret_first("SESSION_COOKIE_SECRET", "FLUENCY_SESSION_SECRET"))
        or "fluency-email-verification"
    )


def _hash_verification_code_digits(six_digits: str) -> str:
    d = re.sub(r"\D", "", str(six_digits or "").strip())
    if len(d) != 6:
        return ""
    return hashlib.sha256(f"{_verification_code_pepper()}:{d}".encode("utf-8")).hexdigest()
...
def _smtp_configured() -> bool:
    host = (_secret_first("SMTP_HOST") or "").strip()
    user = (_secret_first("SMTP_USER", "SMTP_USERNAME") or "").strip()
    pw = (_secret_first("SMTP_PASSWORD") or "").strip()
...
def send_email_verification_code(
    *,
    ...
    """Send HTML/text email with OTP via SMTP (e.g. Gmail with an app password)."""
    if not _smtp_configured():
        return False, "SMTP_HOST / SMTP_USER / SMTP_PASSWORD are not set in secrets."
    host = (_secret_first("SMTP_HOST") or "smtp.gmail.com").strip()
    port = _coerce_int(_secret_first("SMTP_PORT"), 587)
    user = (_secret_first("SMTP_USER", "SMTP_USERNAME") or "").strip()
    password = (_secret_first("SMTP_PASSWORD") or "").strip()
    from_addr = (_secret_first("SMTP_FROM", "SMTP_USER") or user).strip()
    msg = EmailMessage()
    ...
    body = (
        ...
        f"Enter this code on the app sign-in page to verify your email.\n"
...
def register_workspace(
    *,
    ...
    When verification is on, a 6-digit code is emailed via SMTP (hashed in DB).
    """
    ...
    vexp: datetime | None = None
    if verify_on:
        otp_plain = _generate_email_otp()
        otp_hash = _hash_verification_code_digits(otp_plain)
        vexp = datetime.now(timezone.utc) + timedelta(hours=hrs)
    ...
    if verify_on and otp_plain:
        ...
        extra = {
            "masked_email": _mask_email(email_n),
            "expires_hours": hrs,
            "email_sent": ok_send,
        }
        if ok_send:
...
_PAGE_LABELS = ["✨ Generator", "📂 Library", "⚙️ Settings", "🔒 Privacy Policy"]
_PAGE_MAP = {"✨ Generator": "Generator", "📂 Library": "Library", "⚙️ Settings": "Settings", "🔒 Privacy Policy": "Privacy"}
...
if api_key and candidate_sop_for_flowchart:
    ...
    st.caption("Last updated: May 11, 2026")
...
- **Company** (referred to as either "the Company", "We", "Us" or "Our") refers to Fluency.
...
- **Website** refers to Fluency, accessible from [https://sop-generator-79adkbtf3mgofxua2lxwje.streamlit.app/](https://sop-generator-79adkbtf3mgofxua2lxwje.streamlit.app/)
...
Usage Data is collected automatically when using the Service. This may include your Device's IP address, browser type, browser version, the pages of our Service that you visit, the time and date of your visit, time spent on those pages, unique device identifiers, and other diagnostic data.
...
- **Necessary / Essential Cookies** — Session Cookies essential to provide services through the Website.
- **Cookies Policy / Notice Acceptance Cookies** — Persistent Cookies that identify if users have accepted cookies.
- **Functionality Cookies** — Persistent Cookies that remember choices you make (login details, language preference, etc.).
...
- **To contact you** by email or other electronic communication regarding updates or informative communications.
...
- **User Accounts**: retained for the duration of the account relationship plus up to 24 months after closure.
...
Your information may be transferred to and maintained on computers located outside of your jurisdiction where data protection laws may differ. The Company will take all steps reasonably necessary to ensure your data is treated securely.

---

## Delete Your Personal Data

You have the right to delete or request that we assist in deleting the Personal Data we have collected about you. You may update, amend, or delete your information at any time by signing in to your Account.

---

## Security of Your Personal Data

The security of your Personal Data is important to us, but no method of transmission over the Internet or electronic storage is 100% secure. We strive to use commercially reasonable means to protect your Personal Data.
...
- **By email:** contact@fluency-sop.com
- **By visiting:** [https://sop-generator-79adkbtf3mgofxua2lxwje.streamlit.app/](https://sop-generator-79adkbtf3mgofxua2lxwje.streamlit.app/)
""")

    st.divider()
    st.caption("Full hosted version: [Privacy Policy on TermsFeed](https://www.termsfeed.com/live/dcd7fd25-8121-4360-adb7-f190973762c6)")
